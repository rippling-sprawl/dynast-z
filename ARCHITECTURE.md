...
- **Caching / cache busting.** CSS and JS are served `immutable` for a year, so their URLs must change when their contents do. `build.py` appends a content hash (`?v=<hash8>`) at **deploy time** — Vercel runs it via `buildCommand`. Those hashes are **never committed**: source HTML references assets unversioned, and `build.py --check` fails if one sneaks in (`--strip` undoes a stray local run). Never hand-edit a `?v=` value. HTML itself is `no-store`, not `must-revalidate`, because Vercel static hosting sends no `ETag` and a constant `Last-Modified` — there is no working validator, so a revalidating browser could be pinned to an old page (and therefore to old asset URLs). `no-store` guarantees one refresh always lands on the current build. **The one exception is `/football/action`**, which is rendered by a function rather than served as a file — a function *can* send a real validator, so it sends a strong `ETag` (the content hash of the page model) with `max-age=0, must-revalidate`, and an unchanged page costs a 304 instead of a re-download.
